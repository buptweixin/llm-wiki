...
## [2026-09-07] review | GenLIP

- 第 2 次复测：大幅进步，骨架与动机（目标错位、一塔两用生成式预训练）首答即中。
- 上次卡壳核心突破：attention sink 梯度路由机制（对比学习全局池化硬约束 vs 生成式因果预测局部偷懒捷径）首答满分；两阶段低分辨率吃海量数据 + 高分辨率原生宽高比补细粒度回答精准。
- 细节收敛：澄清序列必须是图在前、文本在后，因果约束下图像绝不能看文本；推理阶段丢掉文本后，Prefix-LM 退化回最原始的标准 ViT。
- 判定：通过；复测间隔递推进 +1 月，下次复测 2026-10-07。

## [2026-09-07] review | LaSt-ViT

- 第 2 次复测：核心机制大幅巩固。上次补充的特征通道维 1D FFT 滤波 vs 空间维滤波（空间低频是背景大色块、通道低频是语义一致前景）首答满分；零参数与 Top-K 均值聚合全对；Register 治标不治本（高范数是晚期症状，病根未除导致 PiB 倒跌）明确。
- 攻克三次遗忘老卡壳点「偷懒聚合两大帮凶」：通过 CNN vs ViT 感受野对比彻底钉死（背景多提供偷懒动机 + 全局注意力提供把前景语义广播到背景的作案通道，窗口注意力砍通道 PiB 暴涨但分类掉 8% 铁证），显微镜大前景反向推演全对，彻底收敛。
- 判定：通过；复测间隔递推进 +1 月，下次复测 2026-10-07。

## [2026-09-07] review | Video-o3

- 第 2 次复测：核心直觉唤醒迅速。原生工具调用与 VST 推理时机对比（查询后 vs 查询前）首答焊牢。
- 细节重温与收敛：
  - 为什么必须在生成答案时屏蔽全局视频（TDAM 双向锁之一）：彻底讲清全局低分辨率模糊信息带跑局部高清证据、导致 Fake Thinking 的根因；
  - 澄清 10% 的加掩码比例是防丧失全局局部综合能力的平衡点；
  - VTGR 奖励机制打通：准（裁剪区间 IoU 对齐分）× 快（轮数衰减折扣），成功反推若无轮数衰减模型将无休止调用工具导致上下文膨胀崩溃。
- 判定：通过；复测间隔递推进 +1 月，下次复测 2026-10-07。

## [2026-09-07] review | U-OPSD

- 第 2 次复测：极高质量通过。后训练去监督谱系（SFT→OPD→OPSD→U-OPSD）与双门控机制（过门控自发挑能力边界课程）首答全对。
- 上次核心卡壳点满分突破：反向 KL（Reverse KL）的 mode-seeking 导致复读机塌缩（生成长度从 2.7k 暴涨至 99k，死循环无法终止）机制精准讲透；完整轨迹 $y^+$ 的必要性（label-only 丢单值无法引导中间每一步）回答正确；稠密逐 token 上下文相较于标量 RL 稀疏信号的优势彻底钉死。
- 跨篇认知爆发：自主推演出与 S²VOPD 散度排序颠倒（fKL 最好 vs JSD 最好）的深层物理原因——文本推理属于自身可恢复的内部逻辑，而退化视觉损失是物理不可逆的真实信息丢失，强推前向 KL 导致盲猜幻觉。
- 判定：通过；复测间隔递推进 +1 月，下次复测 2026-10-07。

## [2026-09-07] review | S²VOPD

- 首次复测：高分通过。核心思路（从学生端减信息自造不对称性，零标注零特权）首答即中；JSD 折中散度与对称自蒸馏反噬（跌至 65.21 < 70.58，放大过去自信错误）回答深刻。
- 习惯纠偏：彻底纠正了此前在 U-OPSD 上的习惯性口误，首答准确阐述 $y^+$ 不是伪标签，而是一整条解题思考轨迹先验上下文。
- 难点攻克：通过 Crop 越难越单调暴跌（71.53→68.76→67.44）与冷冻基座教师仍拿 93% 增益两大硬核消融，彻底证伪“练难考易”的直觉误区，钉死“向信息完整版自己对齐”的本质；双退化实验反推全对。
- 判定：通过；复测间隔递推进 +1 周，下次复测 2026-09-14。

## [2026-09-07] query | 站点阅读体验改造 Spec

- 按用户要求将站点体验分析整理为 [详细实施 spec](docs/specs/site-reading-redesign.md)，覆盖页面结构、标签与关系、全文搜索、复测状态、静态实现、迁移阶段和验收矩阵。
- 在 index.md 登记为维护文档；未修改知识正文、现行 schema 或站点实现，未部署。

## [2026-09-07] query | 站点阅读体验改造

- 按 spec 落地连续阅读、语义目录、可点击标签、专题筛选、静态全文搜索片段、回忆模式和 U-OPSD / S²VOPD 并排比较入口。
- 保留原有论文 URL、旧 slide 锚点和 Markdown 真源；新增受控标签词表与静态搜索索引，未部署。

## [2026-09-07] query | 站点阅读改造验收

- 对提交 b35b7b2 进行源码审查和 Chrome 本地交互验收，形成 [验收报告](docs/reviews/site-reading-redesign-review-2026-09-07.md)，结论为需要修改。
- 记录 3 项 P1、6 项 P2 问题及 25 项 spec 对照；只新增报告并登记索引、日志，未修改站点实现或部署。

## [2026-09-07] review | LocateAnything

- 首次复测：高分通过。痛点（逐 token 串行延迟 + 误差滚雪球）与 PBD 核心机制（固定长切块、填空卷一步出完整框、几何整体先验）首答精准。
- 三道深度检验题全部过关：
  1. 歧义触发器：准确阐述“撕裂分布”（top-1 不自信且 top-5 极差大说明候选范围撕裂），双边界反推（微调犹豫 vs 极度自信）全部清晰；
  2. 消融归因：准确抓住核心机制是并行训练时的填空目标（Lblk）强行把几何联合约束压进共享权重，使得 Slow 逐词接龙也能吃到红利（纠偏细节点：单有块排版不加填空损失为 0 增益）；
  3. 通用 MTP 缺陷：直击本质，结构无关切块切断完整语义、迫使模型拟合虚假相关。
- 判定：通过；复测间隔递推进 +1 周，下次复测 2026-09-14。

## [2026-09-07] query | 站点阅读改造返工（验收报告 R1~R9 全项修复）

- 按验收报告修复九项：纠正索引/比较中的反向机制解释并把可恢复性假说独立标注（R1）；重建 wiki-index.js 全文条目索引（每篇约 10~13 条，条目文本取自真源、锚点对账）+ 新增 9 篇完整笔记阅读页 site/notes/papers/ 与 _note-template（R2）；回忆模式改为「只见标题→显示问题→逐题展开」状态机并接入首页/顶部入口（R3）；标签接入 taxonomy.md 受控 ID 与别名归一（R4）；复测到期改为结构化日期按 Asia/Shanghai 判断、S²VOPD y+ 历史卡壳补当时日期与当前状态（R5）；速览页 data-section 语义 ID + qa 稳定锚点，CLAUDE.md / ingest / review 指令同步（R6）；封面与首页压缩、手机目录默认折叠、关系旁栏改提炼卡、侧栏去独立滚动（R7）；比较改原生 dialog（焦点圈定/恢复、依据回链、窄屏按维度重排）（R8）；文章返回入口保留来源筛选（R9）。
- 9 篇 markdown 增加 front-matter（topic/mechanisms/goals 标签 ID），LocateAnything 速览补录 2 条缺失卡壳问答；node 逻辑自测与浏览器（127.0.0.1）抽查全部通过；未部署（deploy 待用户确认后执行）。

## [2026-09-08] query | 站点阅读改造第二轮验收

- 对提交 3c7f95a 复验原 R1~R9：五项具体问题已修复；仍有 2 项 P1、5 项 P2，涉及全文覆盖与实际落点、完整笔记及筛选返回、问答遮挡、手机入口和 VST 标签遗漏。
- Chrome file:// 检查九篇速览及九篇完整笔记 390px 布局，验证回忆状态、比较焦点、上海时区日期边界及六至八节模板；详见 docs/reviews/site-reading-redesign-review-2026-09-08.md。
- 新增第二轮报告并同步 index.md；未修改站点实现、知识正文或复测记录，未提交或部署。

## [2026-09-08] query | 站点阅读改造第三轮验收

- 对未提交的 F1~F7 修复做 Chrome file:// 与本地 HTTP 验收：九篇 27 个新真源查询全部命中，80 个正文章节被索引，生成器在临时副本中重建一致且幂等；九篇正常手机首屏与标签同步通过。
- 整体验收不通过：四项 P2，涉及完整笔记丢弃主题/标签、全文只定位整章、手机回忆泄漏核心提示、比较同页依据仍被模态层遮挡；详见 docs/reviews/site-reading-redesign-review-2026-09-08-round-3.md。
- 仅新增报告并同步维护索引/日志，保留全部未提交实现；未修改知识或复测记录，未提交或部署。

## [2026-09-08] query | 站点阅读改造第四轮验收

- 按第三轮第 6 节进行 Chrome file:// 与本地 HTTP 验收：T1 来源四字段往返、T3 九篇手机回忆、T4 两篇比较的依据与键盘路径通过。
- 整体不通过，T2 仍有跨行内标签短语找不到和多词查询只定位首词两种失败；8 个定位查询中 6 个通过、2 个失败，复现与修复标准见 [第四轮报告](docs/reviews/site-reading-redesign-review-2026-09-08-round-4.md)。
- 仅新增报告并同步 index.md、log.md；保留全部未提交站点实现，未修改知识正文、静态索引或复测记录，未提交或部署。

## [2026-09-08] query | 站点阅读改造第五轮验收

- file:// 与本地 HTTP 重跑第四轮 8 个定位查询均通过，补测跨 code／链接及常见首词新样本；T1 状态往返、T3 回忆隐藏和 T4 比较焦点回归抽查通过。
- 整体不通过：375×667 下 `reward 信号` 的完整短语被前段局部窗口抢占，目标仍在屏外，记为 T2-c / P2；另记录既有高亮标签损坏 P3，不作为阻断。详见 [第五轮报告](docs/reviews/site-reading-redesign-review-2026-09-08-round-5.md)。
- 仅新增报告并同步维护索引、日志，保留全部未提交实现及第四轮记录，未修改知识正文、静态索引或复测记录，未提交或部署。

## [2026-09-08] query | 站点阅读改造第六轮验收

- 验收通过：file:// 与 HTTP、375×667 与 390×844 下，`reward 信号` 实际选中 U-OPSD 段完整短语且初次点击可见；`token 汇聚` 也改为完整短语级命中，T2-c 关闭。
- 第四轮 8 个查询与第五轮 10 个样本全部通过，VST 高亮片段与真源一致且无标签残片，P3 关闭；T1/T3/T4 双通道回归抽查通过。详见 [第六轮报告](docs/reviews/site-reading-redesign-review-2026-09-08-round-6.md)。
- 仅新增报告并同步维护索引、日志，保留所有未提交实现及历史记录，未修改知识正文、静态索引或复测记录，未提交或部署。

## [2026-09-08] query | 站点阅读改造分析与修复交接

- 按用户要求保存对 087cb87 的审查意见：四项 P2 涉及搜索与回忆冲突、关联来源丢失、目录与搜索定位冲突、索引部分再生，并给出三项后续优化及验收标准。
- 记录静态校验、临时副本与路由逻辑复现的证据，明确本次浏览器验证受限；详见 [修复交接文档](docs/reviews/site-reading-redesign-analysis-2026-09-08.md)。
- 仅新增维护文档并同步 index.md、追加本日志；未修改站点实现、知识正文或复测记录，未提交或部署。

## [2026-09-08] lint | 站点阅读 S1~S4 修复与 O1~O3 优化

- 按修复交接文档完成四项 P2：S1 回忆模式标题入口统一速览、S2 站内继续阅读链接（侧栏/文末/手机/笔记互链/比较跨页依据）继承来源、S3 目录点击与首次搜索定位分离、S4 索引生成器改为真源全量重建+校验（干跑与现有索引逐字一致，临时副本实验验证传播与报错）。
- O1 搜索条目排序优先具体问答（y+ / 流式 掩码 / Dataset-S2 三样例过）；O2 手机首页结果前置+完整笔记目录单行横滑（桌面无回归）；O3 新增 scripts/check-site.mjs 静态回归集（C1~C10 全绿）。
- 浏览器（本地 HTTP，412px/1280px）验收 S1/S2/S3/O1/O2 通过；资源版本升至 20260908-6；详见[修复报告](docs/reviews/site-reading-redesign-fixes-2026-09-08.md)。

## [2026-09-09] query | 站点阅读改造第七轮验收

- 对提交 8d661fe 做 file:// 与 HTTP、手机双尺寸及桌面抽查，S1/S3/O1/O2 通过；此前完整笔记定位、回忆隐藏和比较键盘抽查未见回归，O3 静态检查已交付但回归集与样式收拢部分未完成。
- 整体不通过：S2-a 文末 SOURCE 完整笔记出口丢来源；S4-a 完整笔记锚点缺失时重建静默删条目，随后静态检查错误地通过。临时副本确认元数据传播及其他异常检查有效；详见[第七轮报告](docs/reviews/site-reading-redesign-review-2026-09-09-round-7.md)。
- 仅新增报告并同步维护索引、日志；未修改实现、知识正文、静态索引或复测记录，未提交或部署。

## [2026-09-09] lint | 站点阅读改造第七轮收尾

- 修复 S2-a：速览 SOURCE 的完整笔记出口与侧栏、手机入口共用规范化 `from`，保留 hash、四字段与多跳单份来源；资源版本同步为 `20260908-7`。
- 修复 S4-a：生成器对非空真源章节建立完整笔记锚点硬校验；`check-site.mjs` 增加真源章节与派生/索引独立对账，并加入 `scripts/check-site-regressions.mjs` 的单锚点改名正反例。
- O3 登记为**部分完成**：S1/S2/S3 路由组合、短语选择、高亮样例与真源变更实验尚未沉淀为完整交互回归样本；CSS 收拢延期，均留待后续处理。本轮不处理既有 P3 片段文字差异。

## [2026-09-09] ingest | PPO
...
## [2026-09-10] review | 专题：蒸馏与训练预算

- 综合页首次复测满分通过：单教师自蒸馏（去外部高质量标注/模型依赖）与多教师蒸馏（跨域/多教师信用分账与预算错配）两大切片正交性阐述通透；U-OPSD 拼自投票轨迹 $y^+$ 与 S²VOPD 减像素退化图的不对称构造首答精准。
- 专题级机制与跨篇深度考核全部过关：
  1. 反向规则正反馈死循环链条通透：收敛快/差距小（如 IF）被误判为需更多帮助 $\to$ 猛堆预算 $\to$ 差距更小 $\to$ 吞噬更多预算，同时未收敛硬任务被克扣预算导致差距拉大、进一步被减预算，最终导致预算分配发散发狂而崩溃；gap-following 顺应语义让收敛域自动让出预算自带系统刹车；
  2. 散度颠倒深层因果（信息可恢复性）回答堪称教科书级：U-OPSD 的解题逻辑本质可恢复，Forward KL 兼顾全面覆盖，Reverse KL 极化模式塌缩成复读机；S²VOPD 降采样细节物理不可逆丢失，强行 Forward KL 逼学生脑补看不见的细节导致严重幻觉，JSD 折中最佳。
- 派生层与复测队列同步：review.md 专题复测更新为 1 次，下次复测排入 +1 周（2026-09-17）；wiki-index.js 重新生成，静态检查全绿。

## [2026-09-10] review | 专题：视频理解与响应

- 综合页首次复测满分通过：准确切开长视频理解的三笔账分工——VideoChat3 压感知（I3D-ViT 16× 压缩）、VST 调时机（clip 间隙异步思考并维护 FIFO 文本账本）、Video-o3 找证据（全局粗搜 + 自主调用 VideoCrop 细查）。
- 深入辨析时机取舍与组合障碍：
  1. 时机与代价取舍清晰：VST 边看边想、查询即答（0.56s 极低延迟，代价是早期视觉细节有损）；Video-o3 见题才查、多轮收网（证据精准细致，代价是多轮探索耗时长达 10s+）；
  2. 攻克通用大模型融合的底层结构性矛盾：并非简单的“信息谁更可信”，而是底层的「响应时机与控制调度协议冲突」——VST 预设“查询必须立刻吐字”，Video-o3 预设“查询必须挂起去跑探索循环”，两者的动作空间互斥，必须依靠上层动态调度器在用户发问瞬间先做“直出秒答还是挂起探索”的路由决策。
- 派生层与复测队列同步：review.md 视频专题复测更新为 1 次，下次复测排入 +1 周（2026-09-17）；wiki-index.js 重新生成，静态检查全绿。

## [2026-09-14] ingest | TSPO

- 从 Zotero 条目 `QXMPGWGR` 取全文、元数据与 AI Butler 预读；完成 Phase 0 定位、费曼讲解与四题检验。用户理解了 `R_A` 的组级弱监督、`R_T` 的粗粒度定位作用，以及冻结 MLLM 的取舍。
