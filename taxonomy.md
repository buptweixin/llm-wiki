...
| `streaming-inference` | 流式推理 | streaming video |
...
| `on-policy-distillation` | on-policy 蒸馏 | OPD |
| `self-distillation` | 自蒸馏 | self-distillation |
| `multi-teacher` | 多教师蒸馏 | multi-teacher distillation |
| `budget-allocation` | 训练预算 | token budget |
| `parallel-decoding` | 并行解码 | block decoding |
