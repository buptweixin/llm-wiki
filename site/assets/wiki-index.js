...
window.WIKI_INDEX = [
  ...
  },
  {
    ...
    "entries": [
      ...
      },
      {
        ...
        "t": "关联 VST ： 同属长视频推理但路线不同。VST 是\"推理前置\"（播放期边看边想，FIFO 文本记忆），Video-o3 是\"推理时主动检索\"（动态裁剪视频，工具调用）。VST 解决实时性（0.56s），Video-o3 解决多跳精度（46.5% VideoHolmes）。两者可互补：VST 的文本记忆 + Video-o3 的工具裁剪组合；但\"查询即答\"vs\"多轮探索\"在响应时机上逻辑冲突，需统一调度。 GeoAnchor ： 同打破\"纯文本 CoT\"的两条对立路线：Video-o3 把中间推理外化成工具调用（可见可审计、工具可插拔），GeoAnchor 内化成连续潜变量（保几何保真度、教师烧进权重）。本文 Related Works 2.2 自己把\"think with images\"工具流与 latent reasoning 对立。适用分界：动态场景/需人工审计选工具路线，静态图/连续几何精度选 latent 路线。 VideoChat3 ： 视觉策略互补。VideoChat3 靠编码器压缩+状态机决定看多少像素，Video-o3 靠推理时工具调用决定看哪里。一个管感知效率，一个管检索精度。 同领域可对比：Video-R1（文本 CoT，视觉固定）、VideoChat-R1.5/Video-RTS（解耦迭代推理）、VideoZoomer、LOVE-R1。 待建概念页：multi-hop reasoning / tool invocation / attention masking / GRPO / KV cache / test-time scaling"
      }
    ]
  },
  {
  ...
  },
  {
    ...
    "entries": [
      ...
      },
      {
        ...
        "t": "Q：既然可以在同一批数据上跑多个 Epoch，为什么说 PPO 依然是严格的 on-policy 算法？为什么不能丢进 Replay Buffer 长期复用？ PPO 能够多轮复用同一批数据，靠的是 重要性采样比率 $r_t(\\theta)$ 对小幅度策略偏移的纠偏 ，以及 Clip 对过冲梯度的硬性截断 。但这套机制只在当前策略的近端邻域内有效。 如果将陈旧历史数据塞入类似 DQN 的 Replay Buffer 长期复用，会导致两大致命后果： 重要性采样方差指数爆炸 ：新旧策略分布严重脱节，概率比 $r_t(\\theta)$ 极端分化（趋近 0 或膨胀到数万），理论无偏估计彻底失效； 有效梯度几乎全灭（算力彻底空转） ： 旧好动作（$\\hat{A}_t > 0$） ：新策略变聪明了，该动作发生概率自然被拉高（$r_t > 1+\\epsilon$），触发 Clip 上限截断，常数项导数归零； 旧坏动作（$\\hat{A}_t ：新策略学会了规避，该动作发生概率自然被大幅压低（$r_t 也就是说，只有在新策略发生逆向倒退（把好动作概率弄丢、或把坏动作概率拉高）的极少数违背常态的样本上才有纠偏梯度。绝大多数旧样本的梯度全部锁死在常数边界变成了 0，网络空转却学不到任何东西。 因此，这批数据在跑完设定的几个 Epoch（吃口热乎的）后必须彻底丢弃并重新与环境交互采矿，绝不能循环复用陈旧历史。"
      },
      {
  ...
  },
  {
    ...
    "essence": "3D 空间推理别把连续几何量写成文字：推理轨迹改成文本与连续潜变量交错，分解为 position（钉锚）/ direction（箭头）/ geometry（轮廓图）三类 latent 分别监督，四阶段训练最后用 GRPO+pattern reward 学「够用的局部就别拉全局」。",
    "review": {
      "next": "2026-09-17",
      "last": "2026-09-14",
      "count": 0,
    ...
    "relations": [
      {
        "type": "compare",
        "to": "2026-video-o3",
        "reason": "同打破纯文本 CoT 的两条对立路线：Video-o3 把中间推理外化成工具调用（可见可审计、工具可插拔），GeoAnchor 内化成连续潜变量（保几何保真度、教师烧进权重换教师等于重训）",
        "status": "synthesis"
      },
      {
        "type": "compare",
        "to": "2026-locateanything",
        "reason": "同一问题「坐标该不该言语化」的两个答案：LocateAnything 把框写成离散 token 块整块并行解码换效率，GeoAnchor 让几何量不经过词表进连续潜空间换保真度",
        "status": "synthesis"
      },
      {
        "type": "prerequisite",
        "to": "2017-ppo",
        "reason": "Stage 4 的 GRPO 是 PPO 家族的组相对变体（组内均值当 baseline、去 value 网络），PPO 页是理解 pattern reward 的前置",
        "status": "synthesis"
      }
    ],
    "entries": [
      {
        "h": "核心直觉",
        "a": "papers/2026-geoanchor.html#essence",
        "t": "ACM MM'26 双盲投稿。与其逼模型把「物体在 (1.2, 0.5, 3.4)」写成文字再自己读回来，不如允许它推理中途「闭嘴想一想」：把 3D 信息存成 position（钉锚）/ direction（箭头）/ geometry（轮廓图）三类连续潜变量，文本与潜变量交错推理，最后再开口给答案。底座 Qwen3-VL-2B。"
      },
      {
        "h": "一页看懂",
        "a": "papers/2026-geoanchor.html#overview",
        "t": "问题：坐标写成文字经「连续到离散」量化压缩，丢精度还稀释空间语义在注意力里的权重；单 latent（如深度 token）包打不了多样空间任务，只有全局视角缺局部物体证据。方法：文本段与潜变量段交错，position/direction 局部 token 由 3D 坐标和方向显式监督，geometry 全局 token 用 soft coverage 对齐 VGGT；四阶段训练。结果：SPAR-Bench 68.4、SPBench 69.7、跨域 ViewSpatial 47.0 全场第一；但前两个基准与训练数据同源要打折。"
      },
      {
        "h": "机制 · latent token 与三类分解",
        "a": "papers/2026-geoanchor.html#mechanism",
        "t": "latent token 不从词表查表，是模型自己生成的连续 hidden state：position/direction 各 2 步、geometry 8 步。生成的 hidden state 在输出空间，要吃的 embedding 在输入空间，直接塞回去会 latent drift；projector（LayerNorm + MLP 残差）把输出向量翻译回输入空间。三类监督：position 用 Smooth L1（抗离群，GT 来自深度估计有噪声）、direction 用 cosine loss（只管朝向）、geometry 对齐 VGGT。"
      },
      {
        "h": "机制 · soft coverage 与四阶段训练",
        "a": "papers/2026-geoanchor.html#mechanism-2",
        "t": "VGGT 特征是高分辨率网格，geometry token 只有 8 个；严格一一配对会把紧凑表示撕碎。soft coverage 只要求每个特征至少被某个 token 认领 + 8 个 token 工作量均衡（防表示坍缩）。四阶段：S1 局部热身（550k grounding）、S2 联合局部全局、S3 撤监督松弛（latent 弹回语言流形）、S4 GRPO + pattern reward（两种模式各记 EMA 准确率，选高者额外 +0.5，奖励用对模式而非答对）。"
      },
      {
        "h": "卡壳 · 语言先验",
        "a": "papers/2026-geoanchor.html#qa-verbalization",
        "t": "坐标写成文字除了丢精度、稀释注意力权重，还损失语言先验主导：推理全程在文本里走，模型用语言直觉代替几何证据答题。text CoT SFT 不稳定优于 vanilla SFT（三基准平均 51.7 vs 51.8 略降）。"
      },
      {
        "h": "卡壳 · 表示坍缩",
        "a": "papers/2026-geoanchor.html#qa-collapse",
        "t": "去掉 soft coverage 平衡项的极端形态不是一般不均衡，而是表示坍缩：所有 VGGT 特征挤着认领同一个 geometry token，其余 7 个闲置，容量退化成 1 个。平衡项本质是分配均匀度的正则。"
      },
      {
        "h": "卡壳 · Stage 3 为什么不会忘光",
        "a": "papers/2026-geoanchor.html#qa-stage3",
        "t": "几何已在权重里；文本 loss 只淘汰对答题无用的 latent 内容，有用的部分因贡献文本 loss 被梯度保留。证据：Fig. 4 显示 S3 收益远超「把 S2 多训一轮」；Fig. 7 的 t-SNE 上 global token 恰落在 VGGT 与 text 之间。"
      },
      {
        "h": "卡壳 · 工具演化成本",
        "a": "papers/2026-geoanchor.html#qa-tools-vs-latent",
        "t": "Video-o3 的工具可插拔（换更好的裁剪或生成工具不用重训模型），GeoAnchor 的「工具」（VGGT、Depth Anything）训练时烧进权重，换几何教师等于重训。GeoAnchor 自己列的适用边界（静态图像、室内场景）也支持动态场景选工具路线。"
      },
      {
        "h": "数字与代价",
        "a": "papers/2026-geoanchor.html#evidence",
        "t": "SPAR-Bench 68.4（基座 32.4，+36.0 但训练采 SPAR 同源要打折）；ViewSpatial 47.0（+10.7，跨域干净证据）；SPBench Abs. 58.7 远低于 SpatialLadder 81.6，绝对距离仍是短板。打折点：局部监督是伪标签（Depth Anything v3 估计，ScanNet 原生传感器深度未用且论文未解释）；Table 5 avg 数字笔误；复现信息缺口（VGGT 是否冻结等未报告）。"
      },
      {
        "h": "关联",
        "a": "papers/2026-geoanchor.html#relations",
        "t": "Video-o3：同打破纯文本 CoT 的对立路线（外化工具 vs 内化潜变量）。PPO：Stage 4 的 GRPO 是 PPO 的组相对变体。LocateAnything：坐标该不该言语化的另一面（离散 token 块 vs 连续潜空间）。库内第一篇 3D 空间推理，开新专题线。"
      },
      {
        "h": "完整笔记 · 类比与直觉",
        "a": "notes/papers/2026-geoanchor.html#intuition",
        "t": "不许念出声的心算：text CoT 是强迫你每一步都把中间数说出口（一说出口就被四舍五入、丢精度，还容易被语言直觉带偏）；latent 推理允许中间步骤留在「感觉层面」，只在最后 verbalize 一次。陌生房间里的两个锚：给两个东西各钉一个锚（position），锚之间拉一根箭头（direction），脑中保留房间整体轮廓图（geometry）。"
      },
      {
        "h": "完整笔记 · 我的复述",
        "a": "notes/papers/2026-geoanchor.html#restatement",
        "t": "Q1：坐标转文字是连续变离散，损失连续空间信息并稀释空间语义在注意力中的权重；text CoT 注意力 top token 是标点和弱语义符号。Q2：dense 是强迫 8 个人各包干一片，紧凑表示被撕碎；去平衡项会分配不均浪费表示空间。Q3：几何知识已学到模型权重里；这一步修复 stage 2 后靠近几何流型远离文本流型，把它拉回去。Q4：o3 外化（跨边界、依赖工具质量），GeoAnchor 内化（保连续性、不可读）；我选 o3，因为可见可审核且工具可更换适合动态场景。"
      },
      {
        "h": "全文 · 解决什么问题",
      ...
      },
      {
        ...
        "t": "卡壳点与解答 Q：verbalization 除了丢精度、稀释注意力权重，还损失了什么？（Q1 补齐） A：语言先验主导：推理全程在文本里走，模型用语言直觉代替几何证据答题。表格证据：text CoT SFT 不稳定优于 vanilla SFT（三基准平均 51.7 vs 51.8 略降）。 Q：去掉 soft coverage 平衡项的极端形态是什么？（Q2 点透） A：不是「有的多有的少」的一般不均衡，而是表示坍缩：所有 VGGT 特征挤着认领同一个 geometry token，其余 7 个闲置，8 个 token 的容量退化成 1 个。平衡项（本质是分配均匀度的正则）防的就是这个病。 Q：Stage 3 为什么不会把几何「忘光」？机制层怎么讲？（Q3 补齐） A：几何已在权重里；文本 loss 只淘汰对答题无用的 latent 内容（留着它们不降 loss、梯度不保留），有用的部分因贡献文本 loss 被梯度留下。证据双件套：Fig. 4（S3 远超「多训一轮 S2」对照）+ Fig. 7（t-SNE 上 global token 落在 VGGT 与 text 之间）。 Q：Video-o3 vs GeoAnchor 还有一层更深的分叉是什么？（Q4 加深） A：工具演化成本：Video-o3 的工具是可插拔的（换更好的裁剪/生成工具不用重训模型），GeoAnchor 的「工具」（VGGT、Depth Anything）是训练时烧进权重的，换几何教师等于重训。GeoAnchor 论文自己列的适用边界（静态图像、室内场景）也支持动态场景选 Video-o3 路线。"
      },
      {
        "h": "全文问答 · verbalization 除了丢精度、稀释注意力权重，还损失了什么？（2026-09-14 首测）",
        "a": "notes/papers/2026-geoanchor.html#qa-verbalization",
        "t": "verbalization 除了丢精度、稀释注意力权重，还损失了什么？（2026-09-14 首测） 语言先验主导 ：推理全程在文本里走，模型用语言直觉代替几何证据答题。表格证据：text CoT SFT 不稳定优于 vanilla SFT（三基准平均 51.7 vs 51.8 略降）。"
      },
      {
        "h": "全文问答 · 去掉 soft coverage 平衡项的极端形态是什么？（2026-09-14 首测）",
        "a": "notes/papers/2026-geoanchor.html#qa-collapse",
        "t": "去掉 soft coverage 平衡项的极端形态是什么？（2026-09-14 首测） 不是「有的多有的少」的一般不均衡，而是 表示坍缩 ：所有 VGGT 特征挤着认领同一个 geometry token，其余 7 个闲置，8 个 token 的容量退化成 1 个。平衡项（本质是分配均匀度的正则）防的就是这个病。"
      },
      {
        "h": "全文问答 · Stage 3 为什么不会把几何「忘光」？机制层怎么讲？（2026-09-14 首测）",
        "a": "notes/papers/2026-geoanchor.html#qa-stage3",
        "t": "Stage 3 为什么不会把几何「忘光」？机制层怎么讲？（2026-09-14 首测） 几何已在权重里；文本 loss 只淘汰对答题无用的 latent 内容（留着它们不降 loss、梯度不保留），有用的部分因贡献文本 loss 被梯度留下。证据双件套：Fig. 4（S3 远超「多训一轮 S2」对照）+ Fig. 7（t-SNE 上 global token 落在 VGGT 与 text 之间）。"
      },
      {
        "h": "全文问答 · Video-o3 vs GeoAnchor 还有一层更深的分叉是什么？（2026-09-14 首测）",
        "a": "notes/papers/2026-geoanchor.html#qa-tools-vs-latent",
        "t": "Video-o3 vs GeoAnchor 还有一层更深的分叉是什么？（2026-09-14 首测） 工具演化成本 ：Video-o3 的工具是可插拔的（换更好的裁剪/生成工具不用重训模型），GeoAnchor 的「工具」（VGGT、Depth Anything）是训练时烧进权重的，换几何教师等于重训。GeoAnchor 论文自己列的适用边界（静态图像、室内场景）也支持动态场景选 Video-o3 路线。"
      },
      {
        "h": "全文 · 还没搞懂",
